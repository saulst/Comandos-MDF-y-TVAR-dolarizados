...
reporte_salvador <- function(base,treshold,vector_variables,pais0){
  ...
  graficos_impulso <- vector_variables %>%
    map(~{
      ...
      tablas_girf <- c(1,2) %>%
        map(~ GIRF(tvar = model,shock = vector,H = 20,R = 40,restrict.to = .x) %>%
              pluck("responses") %>%
              mutate(regimen = .x) %>%
...
resumen_salvador2<-reporte_salvador(base=base, 
                                   vector_variables = vector_variables,
                                   pais0 =  "Salvador", 
                                   treshold=6)

plot(resumen_salvador2$impulsos[[6]])
plot(resumen_salvador2$serie)

impulso_ief_salvador2<-resumen_salvador2$impulsos[[6]]$data
impulso_ief_salvador2<-split(impulso_ief_salvador2, impulso_ief_salvador2$Regime)

write.xlsx(impulso_ief_salvador2, "El Salvador_impulso_ief final.xlsx")

shock_gdp_salva2<-resumen_salvador2$impulsos[[5]]$data
shock_gdp_salva2<-split(shock_gdp_salva2, shock_gdp_salva2$Regime)
write.xlsx(shock_gdp_salva2, "Salvador_shock_gdp final.xlsx")
...
datos_pais <- base%>% 
  ...
  select(vector_variables2)  %>%  # Me quedo solo las variables del mdoelo
  select(-pais)

modelo_salva<-TVAR(datos_pais,
                   lag = 1,
                   nthresh = 1,
                   thDelay = 1,
                   mTh = 6,           # Posición de la variable que determina el regimen
                   trim = 0.1,
                   dummyToBothRegimes = T,  # Si el modelo debe devolver las dummies del regimen
                   trick = "mapply",
                   plot = T)

modelo_salva$nobs_regimes
